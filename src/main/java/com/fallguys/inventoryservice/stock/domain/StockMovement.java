...
public class StockMovement {
    ...
    /**
     * SO 출고로 신규 출고 이동 이력을 생성한다. type=OUTBOUND, reason 없음(정상 흐름), note 없음.
     * delta는 출고 변동량(음수)이고 sourceRef·sourceLineNo는 원천 문서(SO) 라인이다. id·performedAt은 영속 시 채워진다.
     *
     * @throws IllegalArgumentException delta가 음수가 아닐 때
     */
    public static StockMovement createOutbound(String sku, String itemName, ItemUnit itemUnit, Long warehouseId,
                                               int delta, String sourceRef, Integer sourceLineNo, int stockAfter,
                                               String executorEmpNo, String executorName) {
        if (delta >= 0) {
            throw new IllegalArgumentException("출고 변동량(delta)은 음수여야 합니다: " + delta);
        }
        return new StockMovement(null, sku, itemName, itemUnit, warehouseId, delta, MovementType.OUTBOUND, null,
                sourceRef, sourceLineNo, stockAfter, null, executorEmpNo, executorName, null);
    }
