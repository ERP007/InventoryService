 ...
 *
 * 불변식:
 * - 식별 필드(sku·warehouseId·type·executorEmpNo) 필수
...
public class StockMovement {
    ...
    private StockMovement(Long id, String sku, Long warehouseId, int delta, MovementType type,
                          MovementReason reason, String sourceRef, Integer sourceLineNo, int stockAfter,
                          String memo, String executorEmpNo, Instant performedAt) {
     ...
     */
    public static StockMovement createAdjustment(String sku, Long warehouseId, int delta, MovementType type,
                                                 MovementReason reason, int stockAfter, String memo,
                                                 String executorEmpNo) {
        validateAdjustmentDelta(type, delta);
        return new StockMovement(null, sku, warehouseId, delta, type, reason, null, null,
                stockAfter, memo, executorEmpNo, null);
    }

    /** 영속 엔티티에서 도메인 모델을 복원한다(조회). */
    public static StockMovement of(Long id, String sku, Long warehouseId, int delta, MovementType type,
                                   MovementReason reason, String sourceRef, Integer sourceLineNo, int stockAfter,
                                   String memo, String executorEmpNo, Instant performedAt) {
        return new StockMovement(id, sku, warehouseId, delta, type, reason, sourceRef, sourceLineNo,
